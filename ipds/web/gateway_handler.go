...
func (i *gatewayHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	defer func() {
		if r := recover(); r != nil {
			logrus.Error("A panic occurred in the gateway handler!")
			logrus.Error(r)
			debug.PrintStack()
		}
	}()
...
func (i *gatewayHandler) getOrHeadHandler(w http.ResponseWriter, r *http.Request) {
	...
	//convert to ipfs path
	s := path.FromString(parsedPath.String()).Segments()[1]
	cid, err := cid.Parse(s)
	if err != nil {
		webError(w, "hash err", err, http.StatusBadRequest)
		return
	}
	// Resolve path to the final DAG node for the ETag
	resolvedPath, err := i.api.ResolvePath(r.Context(), ipath.IpfsPath(cid))
	switch err {
	case nil:
	...
	dr, err := i.api.Unixfs().Get(r.Context(), ipath.IpfsPath(resolvedPath.Cid()))
	if err != nil {
...
func (i *gatewayHandler) servePretty404IfPresent(w http.ResponseWriter, r *http.Request, parsedPath ipath.Path) bool {
	...
	logrus.Debugf("using pretty 404 file for %s", parsedPath.String())
	w.Header().Set("Content-Type", ctype)
...
func webErrorWithCode(w http.ResponseWriter, message string, err error, code int) {
	...
	if code >= 500 {
		logrus.Warnf("server error: %s: %s", err)
	}
}
